...
pub type PacketCallback = unsafe extern "C" fn(CodecContext, *const u8, c_uint);
...
impl OBSWebRTCCall {
    fn callback(&self, data: bytes::Bytes) {
        let length = data.len().try_into().unwrap();
        unsafe {
            (self.cb)(self.context, data.as_ptr(), length);
        }
    }
}
                        ...
                        tokio::spawn(async move {
                         ...
                         tokio::spawn(async move {
                            ...
                            while let Ok((rtp_packet, _)) = track.read_rtp().await {
                                ...
                                if let Ok(tmp) = payload {

                                    println!("Partition head {}", cached_packet.is_partition_head(&rtp_packet.payload));
                                    println!("Partition tail {}", cached_packet.is_partition_tail(false, &rtp_packet.payload));

                                    println!("{}", rtp_packet.header.sequence_number);

                                 obsclone.callback(tmp);
                                }
                            }
                         });
                    }
